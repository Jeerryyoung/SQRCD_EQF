...
- Future manuscript Data and code availability statements should cite all three resources and the exact release tag.

## Journal-neutral release freeze

1. Pass tests and remove sensitive or non-redistributable inputs.
2. Create a Git tag such as `v1.1.0`.
3. Archive that exact tag on Zenodo and obtain its version-specific DOI.
4. Record the tag, commit SHA and DOI in downstream manuscripts or reuse statements.
5. Keep the concept DOI for discovery and use the version DOI for the exact archived snapshot.
